...
export function renderAppearanceView({ body, settings, services, saveSettings }) {
  ...
  for (const [label, handler] of [['保存主题', () => services.saveTheme?.({ ...settings.theme, enabled: true, css: area.value })], ['停用主题', () => services.disableTheme?.()], ['恢复已保存', () => services.rollbackTheme?.()], ['重置主题', () => services.resetTheme?.()], ['导出主题', () => services.exportTheme?.()]]) { const button = el(doc, 'button', { type: 'button' }, label); button.onclick = () => runAction(handler, services.notice); actions.append(button); }
